...
namespace Wireframe
{
	...
	std::vector<float> buildCollisionWireframeVertices(const ALLParser::AllFile& all)
	{
		...
		for (const ALLParser::CollisionMesh& mesh : all.collisionMeshes)
		{
			const Vector3I& meshOffset = mesh.header.pos;

			for (const ALLParser::CollisionBlock& block : mesh.blocks)
			{
				if (block.enabled != 1)
				{
					continue;
				...
				for (const ALLParser::CollisionPoly& poly : block.polygons)
				{
	...
	void UploadCollisionWireframe(const ALLParser::AllFile& all)
	{
		...
		{
			collisionRenderer.shader = Shader::CreateCollisionShader();
		}
